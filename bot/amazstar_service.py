...
from api.amazstar_api_client import AmazstarApiClient
from bot.amazstar_playwright_fallback import AmazstarPlaywrightFallbackClient
from bot.models import Product
from utils.config import Settings

LOGGER = logging.getLogger("amazstar.service")

Mode = Literal["api_mode", "playwright_mode"]


class AmazstarService:
    def __init__(self, settings: Settings):
        self.settings = settings
        self.api_client = AmazstarApiClient(settings)
        self.playwright_client = AmazstarPlaywrightFallbackClient(settings)
        self.mode: Mode = "api_mode"

    async def start(self) -> None:
        configured_mode = self.settings.amazstar_run_mode
    ...
    async def fetch_products(self) -> list[Product]:
        ...
        if self.mode == "api_mode":
            try:
                ...
                if self.settings.amazstar_run_mode == "api":
                    raise
