...
LOGGER = logging.getLogger("nazar.api")

TOKEN_RE = re.compile(r'name="_token"\s+value="([^"]+)"')
CSRF_META_RE = re.compile(r'<meta name="csrf-token"\s+content="([^"]+)"')
BOOK_URL_RE = re.compile(r'name="_url"\s+type="hidden"\s+value="([^"]+)"')


class NazarApiClient:
    def __init__(self, settings: Settings):
        self.settings = settings
        self._client = httpx.AsyncClient(
            base_url=settings.nazar_domain,
    ...
    async def _get_login_form_token(self) -> str:
        response = await self._client.get(self.settings.nazar_login_url)
        response.raise_for_status()
        match = TOKEN_RE.search(response.text)
        if not match:
            raise RuntimeError("Could not find Laravel form token on login page")
        return match.group(1)

    async def _refresh_csrf_token(self) -> str:
        response = await self._client.get(self.settings.nazar_products_url)
        response.raise_for_status()
        match = CSRF_META_RE.search(response.text)
        if not match:
            raise RuntimeError("Could not find csrf-token meta after login")
    ...
    async def login(self) -> None:
        ...
        payload = {
            "_token": form_token,
            "email": self.settings.nazar_login,
            "password": self.settings.nazar_password,
        }
        response = await self._client.post(self.settings.nazar_login_url, data=payload)
        response.raise_for_status()
    ...
    def _ajax_headers(self, referer: str | None = None) -> dict[str, str]:
        ...
        return {
            ...
            "Referer": referer or self.settings.nazar_products_url,
        }
            ...
            try:
                ...
                while True:
                    ...
                    response = await self._client.post(
                        "/products/list",
                        data=payload,
                        headers=self._ajax_headers(self.settings.nazar_products_url),
                    )
                    response.raise_for_status()
                ...
                LOGGER.warning("api products fetch failed (attempt %s): %s", attempt, exc)
                self._logged_in = False
    ...
    async def _book_once(self, product_id: int) -> dict[str, Any]:
        modal_headers = self._ajax_headers(self.settings.nazar_products_url)
        modal_payload = {"product_id": str(product_id)}
        LOGGER.info(
            "book flow (api) | modal request | product_id=%s | url=/book_product_modal | method=POST | payload=%s | headers=%s",
            product_id,
            modal_payload,
        ...
        modal_url_match = BOOK_URL_RE.search(modal.text)
        endpoint = modal_url_match.group(1) if modal_url_match else f"/product/{product_id}/book"
        book_headers = self._ajax_headers(self.settings.nazar_products_url)
        book_payload = {"product_id": str(product_id)}
        LOGGER.info(
            "book flow (api) | confirm request | product_id=%s | url=%s | method=POST | payload=%s | headers=%s",
            product_id,
            endpoint,
            book_payload,
            self._masked_headers(book_headers),
        )
        booking_response = await self._client.post(
            endpoint,
        ...
        LOGGER.info(
            ...
            self._safe_snippet(booking_response.text),
        )
        if booking_response.status_code >= 400:
            return {
                "success": False,
                "msg": f"Book confirm failed with status {booking_response.status_code}",
                "debug": {
                    "endpoint": endpoint,
                    "method": "POST",
                    "payload": book_payload,
                    "headers": self._masked_headers(book_headers),
                    "response_status": booking_response.status_code,
                    "response_snippet": self._safe_snippet(booking_response.text),
                },
            }
        try:
            return json.loads(booking_response.text)
        ...
        for attempt in range(1, self.settings.max_retries + 1):
            ...
            try:
                ...
                if not current_product:
                    LOGGER.warning(
                        "booking skipped | product_id=%s not found in current products list (likely stale Telegram message)",
                        product_id,
                    )
                    return {
                        "success": False,
                ...
                if current_product.available <= 0:
                    LOGGER.warning("booking skipped | product_id=%s has available=%s", product_id, current_product.available)
                    return {
                        "success": False,
                ...
                elapsed_ms = (time.perf_counter() - started) * 1000
                refreshed_products = await self.fetch_products(
                    country=self.settings.country,
                    length=self.settings.products_page_size,
                )
                ...
                LOGGER.warning("api booking failed (attempt %s) | product_id=%s | %s", attempt, product_id, exc)
                self._logged_in = False
                self._csrf_token = None
                if attempt >= self.settings.max_retries:
                    raise
